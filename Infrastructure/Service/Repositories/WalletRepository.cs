...
namespace Gpay.Infrastructure.Service.Repositories
{
    public class WalletRepository : IWalletRepository
    {
        ...
        public async Task<WalletTransactionHistory> UpdateWalletTransactionHistoryStatus(string transaction_reference, Gpay.Enums.OrderStatus status)
        {
        ...
        public async Task<CreditWalletRequestResponse> ProcessWalletToWalletTransfer(ClientTransferRequest model)
        {
            ...
            try
            {
                string query = "SELECT * FROM \"Orders\" WHERE \"transactionRef\" = @transactionRef";
                using(var conn = dapperContext.GetDbConnection())
                {
                    var data = await conn.QueryFirstOrDefaultAsync<WalletTransferRequest>(query, new
                    {
                        @transactionRef = model.transactionReference
                    });
                var originatorWallet = await dataBaseContext.Wallets.FirstOrDefaultAsync(c => c.account_number == data.sourceAccountNumber);
                ...
                var destinationWallet = await dataBaseContext.Wallets.FirstOrDefaultAsync(c => c.account_number == data.destinationAccountNumber);
                ...
                {
                    originator_accountNumber = data.sourceAccountNumber,
                    transaction_reference = data.transactionReference,
                    destination_accountNumber = data.destinationAccountNumber,
                    ...
                    status = Gpay.Enums.OrderStatus.Pending,
                    narration = !string.IsNullOrWhiteSpace(data.narration) ? data.narration : $"Transfer from {originatorWallet.account_name} to {destinationWallet.account_name}",
                ...
                if (originatorWallet.account_balance < data.amount)
                {
                ...
                originatorWallet.account_balance -= data.amount;
                destinationWallet.account_balance += data.amount;
                dataBaseContext.Wallets.Update(originatorWallet);
                dataBaseContext.Wallets.Update(destinationWallet);
                await dataBaseContext.SaveChangesAsync();

                await this.UpdateWalletTransactionHistoryStatus(transactionHistory.transaction_reference, Gpay.Enums.OrderStatus.Successful);
                res.message = "Transfer successful";
                };
            }
            catch (Exception e)
            {
                await this.UpdateWalletTransactionHistoryStatus(transactionHistory.transaction_reference, Gpay.Enums.OrderStatus.Failed);
        ...
        public async Task<List<Withdrawals>> GetAllPendingWithdrawals()
        {
            ...
            withdrawal = await dataBaseContext.Withdrawals.Where(c => c.status == Gpay.Enums.OrderStatus.Pending).ToListAsync();
            return withdrawal;
        }
         ...
         public async Task<bool> UpdateWithdrawal(Withdrawals withdrawal)
        {
           var withdrawalToUpdate = await dataBaseContext.Withdrawals.FirstOrDefaultAsync(c => c.transactionReference == withdrawal.transactionReference);
            ...
            if (withdrawal.status == Gpay.Enums.OrderStatus.Failed)
            {
